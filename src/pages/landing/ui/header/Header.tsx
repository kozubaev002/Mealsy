import { ChevronDown, ChevronRight, Menu, Search, Send, X } from 'lucide-react'
...
export const Header: React.FC = () => {
	...
	return (
		<>
			<header
				className={`fixed top-0 left-0 w-full bg-white border-b border-[#D9E6CD] z-[100] transition-transform duration-300 ${isVisible ? 'translate-y-0' : '-translate-y-full'}`}
			>
				<div className='max-w-7xl mx-auto px-4 h-[70px] flex items-center justify-between'>
					...
					<div className='flex items-center gap-10'>
						...
						{/* Десктопные ссылки (скрыты на мобилках) */}
						<nav className='hidden md:flex items-center gap-8 text-[16px] font-medium text-[#333]'>
							<a href='/recipes' className='hover:text-[#6ABF69] transition-colors'>Рецепты</a>
							<a href='/handbook' className='hover:text-[#6ABF69] transition-colors'>Справочник</a>
						</nav>
					</div>

					{/* ЦЕНТРАЛЬНАЯ ЧАСТЬ: Поиск (только для десктопа) */}
					...
					<div className='flex items-center'>
						{/* Кнопка Выйти/Войти для десктопа */}
						<button className='hidden md:block bg-[#6ABF69] text-white px-8 py-2 rounded-lg hover:bg-[#5aa359] transition-colors font-medium'>
							Выйти
						</button>
			...
			{/* Мобильное Overlay Menu (остается без изменений, работает только на мобилках) */}
			<div
				className={`fixed inset-0 bg-white z-[110] transition-transform duration-300 ease-in-out ${isMenuOpen ? 'translate-x-0' : 'translate-x-full'} overflow-y-auto md:hidden`}
			>
				<div className='p-5 max-w-md mx-auto'>
					...
					{menuStep === 'main' ? (
						<div className='space-y-6 animate-in fade-in slide-in-from-right-4 duration-300'>
							...
							<div className='text-[18px] text-[#333] font-medium cursor-pointer'>
								Справочник
							</div>
							<div className='text-[18px] text-[#333] font-medium cursor-pointer'>
								Войти
							...
							<div className='pt-4'>
								...
								<div className='flex gap-3'>
									...
									<div className='w-10 h-10 bg-[#6ABF69] rounded-lg flex items-center justify-center text-white cursor-pointer'>
                                        <div className='border-2 border-white rounded-md p-[2px]'>
