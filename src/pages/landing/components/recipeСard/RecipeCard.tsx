...
const RECIPE_CATALOG_ITEMS: Recipe[] = [
  {
    id: 1,
...
const CATALOG_CARD_VARIANTS = {
  hidden: { opacity: 0, y: 30 },
  visible: { opacity: 1, y: 0, transition: { duration: 0.5 } },
}

const CatalogItem: React.FC<{ recipe: Recipe }> = ({ recipe }) => {
  const [isLiked, setIsLiked] = useState(false)
  const [isSaved, setIsSaved] = useState(false)

  return (
    <motion.div
      variants={CATALOG_CARD_VARIANTS}
      className='flex flex-col group'
    >
      {/* Обернул картинку в Link */}
      <Link to={recipe.path} className='relative aspect-[3/4] rounded-3xl overflow-hidden mb-4 shadow-lg bg-gray-100 cursor-pointer'>
        <motion.img
        ...
        <motion.button
          ...
          whileTap={{ scale: 0.9 }}
          onClick={e => {
            e.preventDefault() // Чтобы переход не срабатывал при клике на сохранение
            e.stopPropagation()
            setIsSaved(!isSaved)
          }}
        ...
        <div className='absolute bottom-0 left-0 right-0 h-28 bg-gradient-to-t from-black/90 via-black/40 to-transparent flex items-end justify-between px-5 pb-5 pointer-events-none'>
          <div className='flex items-center gap-2.5 pointer-events-auto'>
            <motion.div
              onClick={e => {
                e.preventDefault() // Чтобы переход не срабатывал при клике на лайк
                e.stopPropagation()
                setIsLiked(!isLiked)
              }}
              whileHover={{ scale: 1.1 }}
...
export function RecipeCatalog() {
  return (
    <section className='max-w-[1240px] mx-auto px-4 py-20 font-sans'>
      ...
      <motion.div
        variants={CATALOG_GRID_VARIANTS}
        initial='hidden'
        whileInView='visible'
        viewport={{ once: true }}
        className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-8 gap-y-16'
      >
        {RECIPE_CATALOG_ITEMS.map(recipe => (
          <CatalogItem key={recipe.id} recipe={recipe} />
        ))}
      </motion.div>

      <div className='mt-24 flex justify-center'>
        {/* Обернул кнопку в Link для перехода на /catalog */}
